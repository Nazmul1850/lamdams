# modqldpc/scheduling/algos/cpsat_v2_scheduling.py
#
# CPSATv2Scheduler — copy of the original CPSATScheduler for independent
# experimentation.  No behavioural changes vs cp_sat_scheduling.py.
# Modify this file freely; the original is left untouched.
#
...
class _CompRole:
    ...
    participant_blocks: init_block_map.keys() ∪ K_LOCAL_COUPLE blocks — used to suppress
                        self-conflicting route-lock intervals (mirrors greedy's definition).
    """
    cid: int
    ...
    is_multiblock: bool
    participant_blocks: frozenset    # frozenset[int]: init + lc blocks
...
def _extract_comp_roles(dag: Any) -> Tuple[List[_CompRole], Dict[str, int]]:
    ...
    for cid, comp in enumerate(components):
        ...
        # Participant blocks = all blocks owned by this component.
        # Includes init_block_map keys AND all K_LOCAL_COUPLE blocks (mirrors greedy).
        _p_blocks: Set[int] = set(init_block_map.keys())
        for nid in comp:
            if dag.nodes[nid].kind == K_LOCAL_COUPLE:
                _p_blocks.update(dag.nodes[nid].blocks)
        ...
        roles.append(_CompRole(
            ...
            participant_blocks=frozenset(_p_blocks),
        ))
...
@dataclass
class CPSATv2Scheduler(BaseScheduler):
    name: str = "cpsat_v2"
    ...
    def solve(self, problem: SchedulingProblem) -> Schedule:
        ...
        if log_search:
            dbg_path = pathlib.Path(__file__).parent / f"cpsat_logs/cpsat_v2_debug{suffix}.txt"
        ...
        # Uses r.participant_blocks which includes K_LOCAL_COUPLE blocks (Fix 2).
        cid_participant_blocks: Dict[int, Set[int]] = {
            r.cid: set(r.participant_blocks) for r in comp_roles
        }
        ...
        # ── Pre-compute route options for all link nodes ─────────────────────
        # Must happen BEFORE horizon computation so we can use max route
        # duration per link node instead of the original (min-path) duration.
        #
        # Bug fix (ring topology): Step 1 previously added  e == s + node_dur
        # for every node including link nodes.  For multi-option links the route
        # selection logic in Step 3 also enforces  e == s + opt.duration  for
        # each chosen option.  When opt.duration ≠ node_dur the two constraints
        # contradict each other, making every longer-route option infeasible.
        # On ring topologies almost every multi-block rotation has two paths
        # (CW and CCW) with different hop counts, so the longer path — which
        # could route around a congested block — is silently eliminated.  The
        # solver is then forced to sequence operations that could run in parallel
        # with the longer route, exploding the search space and causing timeouts.
        precomp_link_routes: Dict[str, List[_RouteOption]] = {}
        if use_routes:
            for nid, node in dag.nodes.items():
                if getattr(node, "kind", "") == K_INTERBLOCK_LINK:
                    precomp_link_routes[nid] = _enumerate_route_options(node, hw, max_hops)

        # ── Node durations ───────────────────────────────────────────────────
        # For link nodes with route alternatives, use the MAX route duration so
        # that the horizon is a valid upper bound for all possible route choices.
        node_dur: Dict[str, int] = {}
        for nid in dag.nodes:
            if nid in problem.duration_override:
                node_dur[nid] = int(problem.duration_override[nid])
            elif nid in precomp_link_routes:
                opts = precomp_link_routes[nid]
                node_dur[nid] = max(opt.duration for opt in opts) if opts else 1
            else:
        ...
        # For link nodes handled by route alternatives (use_routes=True), we do
        # NOT add the fixed  e == s + dur  constraint here.  The route selection
        # logic in Step 3 enforces the correct per-option duration via
        # only_enforce_if constraints.  Adding a fixed constraint here would
        # conflict with any option whose duration differs from node_dur.
        for nid, node in dag.nodes.items():
            dur = node_dur[nid]
            s = model.new_int_var(0, horizon, f"s_{nid}")
            e = model.new_int_var(0, horizon, f"e_{nid}")
            is_link = (getattr(node, "kind", "") == K_INTERBLOCK_LINK)
            if not (use_routes and is_link):
                model.add(e == s + dur)
            start_vars[nid] = s
            end_vars[nid] = e

        # ── Greedy hint (Fix 3) ───────────────────────────────────────────────
        # If a greedy schedule is provided via meta["greedy_hint"], seed CP-SAT
        # with those start/end times so it begins from a known feasible solution
        # and can prove optimality faster.
        greedy_hint: Dict[str, dict] = problem.meta.get("greedy_hint", {})
        if greedy_hint:
            for nid in dag.nodes:
                gh = greedy_hint.get(nid)
                if gh:
                    model.add_hint(start_vars[nid], int(gh["start"]))
                    model.add_hint(end_vars[nid],   int(gh["end"]))
        ...
        for nid, node in dag.nodes.items():
            ...
            if use_routes and is_link:
                # Reuse pre-computed options (computed before horizon, above).
                options = precomp_link_routes.get(nid) or _enumerate_route_options(node, hw, max_hops)
        ...
        for link_nid, role in link_nid_to_role.items():
            ...
            for i, (opt, opt_s) in enumerate(zip(opts, opt_starts)):
                ...
                # Collect all intermediate blocks and all hop edges
                route_blocks_in_paths: Set[int] = set()
                hop_set: Set[Tuple[int, int, int]] = set()
                for path in opt.paths:
                    for b in path:
                        if b not in p_blocks:
                            route_blocks_in_paths.add(b)
                    for h, (u, v) in enumerate(zip(path, path[1:])):
                        hop_set.add((h, u, v))

                # Fix 1: one continuous interval per intermediate block spanning the
                # full link duration.  This matches greedy's ownership semantics —
                # the block is held for the entire routing operation, preventing
                # another rotation from using it mid-link.  (Old code used per-hop
                # 1-timestep intervals, which under-constrained the model.)
                for b in sorted(route_blocks_in_paths):
                    if sel is not None:
                        rb_e = model.new_int_var(0, horizon, f"rbe_{link_nid}_r{i}_{b}")
                        rb_iv = model.new_optional_interval_var(
                            opt_s, opt.duration, rb_e, sel, f"rbiv_{link_nid}_r{i}_{b}"
                        )
                        model.add(rb_e == opt_s + opt.duration).only_enforce_if(sel)
                    else:
                        rb_iv = model.new_interval_var(
                            opt_s, opt.duration, end_vars[link_nid],
                            f"rbiv_{link_nid}_{b}"
                        )
                    block_ivals.setdefault(b, []).append((rb_iv, 1))
                    dbg(f"  [ROUTE_BLOCK]  link={link_nid}  r={i}  block={b}"
                        f"  dur={opt.duration}  optional={sel is not None}")

                # Per-hop intervals for couplers only (coupler used 1 timestep/hop)
                for h, u, v in sorted(hop_set):
                    ...
                    dbg(f"  [HOP_COUPLER]  link={link_nid}  r={i}  h={h}"
        ...
        solver.parameters.log_to_stdout = log_search
        ...
        if not solved:
            ...
            raise RuntimeError(
                f"CP-SAT v2 could not find a feasible schedule (status={status_name}). "
